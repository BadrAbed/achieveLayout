...
class LangReviewerController extends Controller
{
    public function AjaxAllLessons()
    {

        {
            ...
            $data = Content::with('Country', 'grade', 'Categories', 'user')->orderBy("id", "desc")->where(["complete" => 1, 'flowStatus' => CONTENT_FOLLOW_STATUS_ENUMS::UNDER_LANG_CORRECT])->whereNotIn('id', $ckeckRow)->get();
    ...
    public function AjaxMyLessons()
    {

        {
            ...
            //$data = [];
            $ckeckRow = AssignLessonsToReviewers::where('user_id', auth()->id())->get()->pluck('content_id')->toArray();
            $data = Content::with('Country', 'grade', 'Categories', 'user')->orderBy("id", "desc")->where(["complete" => 1, 'flowStatus' => CONTENT_FOLLOW_STATUS_ENUMS::UNDER_LANG_CORRECT])->whereIn('id', $ckeckRow)->get();
    ...
    {
        return view('langReviewer.allLessons');
    }

    public function myLessonsView()
    {
        return view('langReviewer.myLessons');
    }

    public function assignLessonToAudit($id)
    {
        ...
        $newRow->status = CONTENT_FOLLOW_STATUS_ENUMS::UNDER_LANG_CORRECT;
        $newRow->save();
    ...
    public function resendingLessons()
    {
        ...
        $contents = Content::with('Country', 'grade', 'Categories', 'user')->orderBy("id", "desc")->where(["complete" => 1, 'flowStatus' => CONTENT_FOLLOW_STATUS_ENUMS::RESEND_TO_LangREVIEWER])->whereIn('id', $content_ids)->get();
        NotificationsController::markAsRead();
        return view('langReviewer.resendingLessons')->with(compact('contents'));
    }
}
