...
window.addEventListener('load', function (e) {
    ...
    try {
        var args = {
            text_element: document.getElementById('passage-text2'),
            audio_element: document.getElementById('passage-audio2'),
            autofocus_current_word: document.getElementById('autofocus-current-word2').checked
        };
        ...
        if (args.audio_element.networkState === args.audio_element.NETWORK_NO_SOURCE) {
            ...
            document.querySelector('.passage-audio-unavailable2').hidden = false;
            throw new Error('Cannot play any of the available sources');
        }
        ...
        if (supports_playback_rate) {
            var rate_range_element = document.getElementById('playback-rate2');
            rate_range_element.disabled = false;
            rate_range_element.addEventListener('change', function (e) {
                args.audio_element.playbackRate = this.valueAsNumber;
            }, false);
            ...
            document.querySelector('.playback-rate-unavailable2').hidden = false;
        }

        ReadAlong2.init(args);

        document.getElementById('autofocus-current-word2').addEventListener('change', function (e) {
            ReadAlong2.autofocus_current_word = this.checked;
        }, false);

        document.querySelector('.passage-audio2').hidden = false;
        if (supports_playback_rate) {
            document.querySelector('.playback-rate2').hidden = false;
        }
        document.querySelector('.autofocus-current-word2').hidden = false;
    }
    catch (err) {
        console.error(err);
    }
